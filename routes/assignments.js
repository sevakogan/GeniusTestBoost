...
router.get(
  "/pending-grading",
  requireRole("teacher", "owner", "admin"),
  async (req, res) => {
    try {
...
router.get("/course/:courseId", async (req, res) => {
  try {
    ...
    if (role === "student" && assignments && assignments.length > 0) {
      ...
      assignments.forEach((a) => {
        a.my_submission = subMap[a.id] || null;
      });
    }

    if (
      ["teacher", "owner", "admin"].includes(role) &&
      assignments
    ) {
      for (const a of assignments) {
        ...
        a.ungraded_count = subs
          ? subs.filter((s) => s.grade === null).length
          : 0;
      }
    }
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "teacher") {
        const owns = await verifyTeacherOwns(course_id, req.user.id);
        if (!owns)
          return res.status(403).json({ error: "Not your course" });
      }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "teacher") {
        const owns = await verifyTeacherOwns(
...
router.post("/:id/submit", requireRole("student"), async (req, res) => {
  try {
    ...
    const enrolled = await verifyEnrolled(
      assignment.course_id,
      req.user.id
    );
    if (!enrolled)
